    ...
    , ss(new StoredServer())
{
    ...
    if (ss->load(server) < 0) {
        QMessageBox::information(this,
            ...
            ss->m_last_err.isEmpty() ? tr("部分服务器信息加载失败") : ss->m_last_err);
    }
    ...
    ss->set_protocol_id(0);
    ss->set_protocol_name("anyconnect");
...
void EditDialog::on_buttonBox_accepted()
{
    ...
    if (ui->nameEdit->text().isEmpty() == true) {
        QMessageBox::information(this,
            qApp->applicationName(),
            tr("您需要为此连接指定一个名称，例如： 'My company'"));
        return;
    }
